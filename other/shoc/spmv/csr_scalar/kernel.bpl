...
procedure {:source_name "spmv_csr_scalar_kernel"} {:kernel} $spmv_csr_scalar_kernel($dim: bv32);
...
implementation {:source_name "spmv_csr_scalar_kernel"} {:kernel} $spmv_csr_scalar_kernel($dim: bv32)
{
  var $t.0$1: bv32;
  var $t.0$2: bv32;
  var $j.0$1: bv32;
  ...
  $0:
    ...
    v2$1 := (if p0$1 then $$rowDelimiters[v0$1] else v2$1);
    v2$2 := (if p0$2 then $$rowDelimiters[v0$2] else v2$2);
    v3$1 := (if p0$1 then $$rowDelimiters[BV32_ADD(v0$1, 1bv32)] else v3$1);
    v3$2 := (if p0$2 then $$rowDelimiters[BV32_ADD(v0$2, 1bv32)] else v3$2);
    $t.0$1, $j.0$1 := (if p0$1 then 0bv32 else $t.0$1), (if p0$1 then v2$1 else $j.0$1);
    $t.0$2, $j.0$2 := (if p0$2 then 0bv32 else $t.0$2), (if p0$2 then v2$2 else $j.0$2);
  ...
  $2:
    ...
    v4$1 := (if p1$1 then BV32_SLT($j.0$1, v3$1) else v4$1);
    v4$2 := (if p1$2 then BV32_SLT($j.0$2, v3$2) else v4$2);
    p2$1 := false;
    p2$2 := false;
    ...
    $t.0$1, $j.0$1 := (if p2$1 then FADD32($t.0$1, FMUL32(v6$1, v7$1)) else $t.0$1), (if p2$1 then BV32_ADD($j.0$1, 1bv32) else $j.0$1);
    $t.0$2, $j.0$2 := (if p2$2 then FADD32($t.0$2, FMUL32(v6$2, v7$2)) else $t.0$2), (if p2$2 then BV32_ADD($j.0$2, 1bv32) else $j.0$2);
  ...
  $2.tail:
    assume !p1$1 && !p1$2;
    call {:sourceloc} {:sourceloc_num 11} _LOG_WRITE_$$out(p0$1, v0$1, $t.0$1, $$out[v0$1]);
    ...
    call {:check_id "check_state_0"} {:sourceloc} {:sourceloc_num 11} _CHECK_WRITE_$$out(p0$2, v0$2, $t.0$2);
    assume {:captureState "call_return_state_0"} {:procedureName "_CHECK_WRITE_$$out"} true;
    $$out[v0$1] := (if p0$1 then $t.0$1 else $$out[v0$1]);
    $$out[v0$2] := (if p0$2 then $t.0$2 else $$out[v0$2]);
    return;
