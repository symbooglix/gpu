...
{ 
  ...
  __requires(offset == 0);
	...
	for(i=0;
        __global_invariant(__read_implies(m, (__read_offset_bytes(m)/sizeof(float) - tx) % matrix_dim == 0)),
    ...
    for(i=1;
        __global_invariant(__write_implies(m, (__write_offset_bytes(m)/sizeof(float) - tx) % matrix_dim == 0)),
        __invariant(array_offset == matrix_dim * i),
        i < BLOCK_SIZE; i++){
