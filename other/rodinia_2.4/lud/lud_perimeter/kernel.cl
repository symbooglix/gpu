  ...
  if (tx < BLOCK_SIZE) { //peri-row
    ...
    for(i=1;
        __global_invariant(__implies(__write(m) & (tx < BLOCK_SIZE),
                                     (__write_offset_bytes(m)/sizeof(float) - (tx + (bx+1)*BLOCK_SIZE)) % matrix_dim == 0)),
        __invariant(array_offset == matrix_dim * i),
        i < BLOCK_SIZE; i++){
