...
function runGV()
{
  ...
  # --no-infer - For symbooglix and corral we don't want loop invariants to be inferred. This leads to useless asserts/assumes being in loops.
  #              some of those might not even be correct invariants as GPUVerify relies on houdini to remove non inductive invariants later on
  # --only-require - We want manually loop invariants to be removed but we want any requires on kernel parameters to remain
