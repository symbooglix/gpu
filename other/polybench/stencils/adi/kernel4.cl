...
__kernel void kernel4(__global double *A, __global double *B, int n, int tsteps, int h0)
{
    ...
    for (int g1 = 32 * b0;
         ...
         __global_invariant(__write_implies(B, (((__write_offset_bytes(B)/sizeof(double)) - t0) % n) == 32 * b0)),
         __global_invariant(__read_implies(B, (((__read_offset_bytes(B)/sizeof(double)) - t0) % n) == 32 * b0)),
         g1 < n; g1 += 1048576) {
      for (int g5 = 0;
           __global_invariant(__implies(__same_group, __no_write(shared_A))),
           __global_invariant(__implies(__same_group, __no_read(shared_A))),
        ...
        if (n >= t0 + g1 + 1) {
          for (int c2 = max(-g5 + 1, 0);
            ...
            B[(g5 + c2) * n + (t0 + g1)] = (B[(g5 + c2) * n + (t0 + g1)] - (( shared_A[c2][t0] * shared_A[c2][t0] ) / B[(g5 + c2 - 1) * n + (t0 + g1)]));
          }
        }
        barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
      }
