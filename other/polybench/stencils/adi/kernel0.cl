...
__kernel void kernel0(__global double *A, __global double *B, int n, int tsteps, int h0)
{
    ...
    for (int g1 = 32 * b0;
         __global_invariant(__write_implies(B, (((__write_offset_bytes(B)/sizeof(double))/n) % 1048576) == 32 * b0 + t0)),
         __global_invariant(__read_implies(B, (((__read_offset_bytes(B)/sizeof(double))/n) % 1048576) == 32 * b0 + t0)),
      ...
      for (int g5 = 0;
        ...
        if (n >= t0 + g1 + 1) {
          for (int c2 = max(-g5 + 1, 0);
              ...
              B[(t0 + g1) * n + (g5 + c2)] = (B[(t0 + g1) * n + (g5 + c2)] - ((shared_A[t0][c2] * shared_A[t0][c2]) / B[(t0 + g1) * n + (g5 + c2 - 1)]));
          }
        }
        barrier(CLK_LOCAL_MEM_FENCE | CLK_GLOBAL_MEM_FENCE);
      }
