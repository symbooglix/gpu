//pass
//--gridDim=[2,128] --blockDim=[16,4] --no-inline
           ...
           )
{
    ...
    for (ib = threadIdx.x;
         ...
         __global_invariant(__write_implies(LocalBlock, (__write_offset_bytes(LocalBlock)-SharedIdx) >= 0)),
    ...
    if (threadIdx.y < RADIUS*2)
    {
        ...
        for (ib = threadIdx.x;
             ...
             __global_invariant(__implies(threadIdx.y >= RADIUS*2, !__enabled())),
             __global_invariant(__implies(threadIdx.y < RADIUS*2, __write_implies(LocalBlock,
                  (((__write_offset_bytes(LocalBlock)-SharedIdx) >= 0)
                ...
                | (((__write_offset_bytes(LocalBlock)-SharedIdxOld) >= 0)
