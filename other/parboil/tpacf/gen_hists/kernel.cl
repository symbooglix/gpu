//pass
//--local_size=[256] --num_groups=[201] --warp-sync=32
    ...
    __constant float* dev_binb, int NUM_SETS, int NUM_ELEMENTS)
{
  ...
  for(unsigned int w = 0; __invariant(__write_implies(warp_hists,(__write_offset_bytes(warp_hists)/sizeof(unsigned int)) % BLOCK_SIZE == get_local_id(0))), w < NUM_BINS*NUM_HISTOGRAMS; w += BLOCK_SIZE )
  {
  ...
  for(unsigned int offset = NUM_HISTOGRAMS >> 1;
      offset > 0;
      offset >>= 1)
  {
    for(unsigned int bin_base = 0;
        bin_base < NUM_BINS;
