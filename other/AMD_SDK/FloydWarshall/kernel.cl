//pass
//--local_size=[8,8] --num_groups=[8,8] --equality-abstraction
...
{
	__requires(numNodes == 8*8 /*get_global_size(0)*/);
	__requires(pass == 0); //< previously, 0 <= pass < numNodes
	
    int xValue = get_global_id(0);
    int yValue = get_global_id(1);

    int k = pass;
    /*int*/ unsigned oldWeight = pathDistanceBuffer[yValue * numNodes + xValue];

    // Kernel is not data race free unless we assume that addition does not overflow
    /*int*/ unsigned tempWeight = __add_noovfl_unsigned_int(pathDistanceBuffer[yValue * numNodes + k], pathDistanceBuffer[k * numNodes + xValue]);
