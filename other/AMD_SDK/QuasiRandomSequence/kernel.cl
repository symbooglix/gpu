...
#ifndef ORIGINAL
        // AD: Strictly no need for the following loop to be executed only
        // by the master thread: if all threads execute it all that happens
        // is that there are benign races
        if(get_local_id(0) == 0) {
#endif
	for(int i=0;
#ifndef ORIGINAL
            __global_invariant(__write_implies(shared, get_local_id(0) == 0)),
#endif
            i < N_DIRECTIONS; i++)
	{
...
#ifndef ORIGINAL
        // AD: The barrier is not stricly required, and not present in the original,
        // because without it all that one gets is a benign race between the prior loop
