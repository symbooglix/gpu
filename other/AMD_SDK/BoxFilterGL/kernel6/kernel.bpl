...
procedure {:source_name "box_filter_vertical"} {:kernel} $box_filter_vertical($filterWidth: bv32);
...
implementation {:source_name "box_filter_vertical"} {:kernel} $box_filter_vertical($filterWidth: bv32)
{
  var $sum.0$1: bv128;
  var $sum.0$2: bv128;
  var $Y.0$1: bv32;
  ...
  $0:
    ...
    v2$1 := BV32_ADD(BV32_ADD(BV32_MUL(group_size_x, group_id_x$1), local_id_x$1), BV32_MUL(v0$1, v1));
    v2$2 := BV32_ADD(BV32_ADD(BV32_MUL(group_size_x, group_id_x$2), local_id_x$2), BV32_MUL(v0$2, v1));
    ...
    $sum.0$1, $Y.0$1 := (if p4$1 then 0bv128 else $sum.0$1), (if p4$1 then BV32_SUB(0bv32, v3) else $Y.0$1);
    $sum.0$2, $Y.0$2 := (if p4$2 then 0bv128 else $sum.0$2), (if p4$2 then BV32_SUB(0bv32, v3) else $Y.0$2);
  ...
  $4:
    ...
    $sum.0$1, $Y.0$1 := (if p6$1 then BV32_ADD(BV32_ADD($sum.0$1[128:96], BV8_ZEXT32(v10$1)), BV8_ZEXT32(v14$1)) ++ BV32_ADD(BV32_ADD($sum.0$1[96:64], BV8_ZEXT32(v9$1)), BV8_ZEXT32(v13$1)) ++ BV32_ADD(BV32_ADD($sum.0$1[64:32], BV8_ZEXT32(v8$1)), BV8_ZEXT32(v12$1)) ++ BV32_ADD(BV32_ADD($sum.0$1[32:0], BV8_ZEXT32(v7$1)), BV8_ZEXT32(v11$1)) else $sum.0$1), (if p6$1 then BV32_ADD($Y.0$1, 2bv32) else $Y.0$1);
    $sum.0$2, $Y.0$2 := (if p6$2 then BV32_ADD(BV32_ADD($sum.0$2[128:96], BV8_ZEXT32(v10$2)), BV8_ZEXT32(v14$2)) ++ BV32_ADD(BV32_ADD($sum.0$2[96:64], BV8_ZEXT32(v9$2)), BV8_ZEXT32(v13$2)) ++ BV32_ADD(BV32_ADD($sum.0$2[64:32], BV8_ZEXT32(v8$2)), BV8_ZEXT32(v12$2)) ++ BV32_ADD(BV32_ADD($sum.0$2[32:0], BV8_ZEXT32(v7$2)), BV8_ZEXT32(v11$2)) else $sum.0$2), (if p6$2 then BV32_ADD($Y.0$2, 2bv32) else $Y.0$2);
