...
implementation {:source_name "kernel8"} {:kernel} $kernel8($m: bv32, $n: bv32)
{
  ...
  __partitioned_block_$1_0:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b20 ==> _WRITE_HAS_OCCURRED_$$kernel8.shared_symmat ==> _WATCHED_OFFSET == BV32_ADD(BV32_MUL(local_id_x$1, 32bv32), local_id_x$1);
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b19 ==> _READ_HAS_OCCURRED_$$kernel8.shared_symmat ==> _WATCHED_OFFSET == BV32_ADD(BV32_MUL(local_id_x$1, 32bv32), local_id_x$1);
    assert {:tag "nowrite"} _b18 ==> !_WRITE_HAS_OCCURRED_$$kernel8.shared_symmat;
    assert {:tag "noread"} _b17 ==> !_READ_HAS_OCCURRED_$$kernel8.shared_symmat;
    assert {:tag "accessBreak"} _b16 ==> _WRITE_HAS_OCCURRED_$$symmat ==> local_id_x$1 == BV32_DIV(_WATCHED_OFFSET, $m);
    assert {:tag "nowrite"} _b15 ==> !_WRITE_HAS_OCCURRED_$$symmat;
