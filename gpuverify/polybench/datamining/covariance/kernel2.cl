...
__kernel void kernel2(__global double *symmat, int m, int n)
{
    __requires(m == 1024);
    __requires(n == 1024);
    ...
    for (int g1 = 32 * b0; g1 < m; g1 += 8192)
      for (int g3 = 32 * b1 + 8192 * floord(-32 * b1 + g1 - 1, 8192) + 8192; g3 < m; g3 += 8192) {
        for (int c1 = max(t1 + 16 * floord(t0 - t1 + g1 - g3 - 1, 16) + 16, t1); c1 <= min(m - g3 - 1, 31); c1 += 16)
