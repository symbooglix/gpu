...
implementation {:source_name "kernel0"} {:kernel} $kernel0($ny: bv32, $nx: bv32)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b59 ==> _WRITE_HAS_OCCURRED_$$kernel0.shared_x ==> _WATCHED_OFFSET == local_id_x$1;
    assert {:tag "nowrite"} _b58 ==> !_WRITE_HAS_OCCURRED_$$kernel0.shared_x;
    assert {:tag "noread"} _b57 ==> !_READ_HAS_OCCURRED_$$kernel0.shared_x;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b56 ==> _WRITE_HAS_OCCURRED_$$tmp ==> BV32_AND(BV32_SUB(1048576bv32, 1bv32), _WATCHED_OFFSET) == BV32_AND(BV32_SUB(1048576bv32, 1bv32), BV32_ADD(BV32_MUL(32bv32, group_id_x$1), local_id_x$1));
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b55 ==> _READ_HAS_OCCURRED_$$tmp ==> BV32_AND(BV32_SUB(1048576bv32, 1bv32), _WATCHED_OFFSET) == BV32_AND(BV32_SUB(1048576bv32, 1bv32), BV32_ADD(BV32_MUL(32bv32, group_id_x$1), local_id_x$1));
    assert {:tag "nowrite"} _b54 ==> !_WRITE_HAS_OCCURRED_$$tmp;
    assert {:tag "noread"} _b53 ==> !_READ_HAS_OCCURRED_$$tmp;
  ...
  __partitioned_block_$7_0:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b81 ==> _WRITE_HAS_OCCURRED_$$kernel0.shared_x ==> _WATCHED_OFFSET == local_id_x$1;
    assert {:tag "nowrite"} _b80 ==> !_WRITE_HAS_OCCURRED_$$kernel0.shared_x;
    assert {:tag "noread"} _b79 ==> !_READ_HAS_OCCURRED_$$kernel0.shared_x;
    ...
    assert p6$1 ==> p0$1;
    assert p6$2 ==> p0$2;
    assert {:do_not_predicate} {:tag "accessOnlyIfEnabledInEnclosingScopes"} {:thread 1} _b16 ==> _WRITE_HAS_OCCURRED_$$kernel0.shared_x ==> BV32_SLT($g1.0$1, $nx);
    assert {:do_not_predicate} {:tag "accessOnlyIfEnabledInEnclosingScopes"} {:thread 1} _b15 ==> _READ_HAS_OCCURRED_$$kernel0.shared_x ==> BV32_SLT($g1.0$1, $nx);
  ...
  $20:
    ...
    assert p6$1 ==> p0$1;
    assert p6$2 ==> p0$2;
    assert {:do_not_predicate} {:tag "accessOnlyIfEnabledInEnclosingScopes"} {:thread 1} _b26 ==> _READ_HAS_OCCURRED_$$kernel0.shared_x ==> BV32_SLT($g1.0$1, $nx) && BV32_SLE($g5.0$1, $0$1) && BV32_SGE($nx, BV32_ADD(BV32_ADD(local_id_x$1, $g1.0$1), 1bv32));
    ...
    assert {:do_not_predicate} {:tag "conditionalLoopExecution"} {:thread 1} _b23 ==> !BV32_SGE($nx, BV32_ADD(BV32_ADD(v0$1, $g1.0$1), 1bv32)) ==> !_READ_HAS_OCCURRED_$$kernel0.shared_x;
