...
implementation {:source_name "kernel1"} {:kernel} $kernel1($length: bv32, $tsteps: bv32, $h0: bv32)
{
  ...
  $1:
    ...
    assert {:tag "nowrite"} _b31 ==> !_WRITE_HAS_OCCURRED_$$c;
    ...
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b7 ==> BV32_SLT($g1.0$1, $length) ==> p0$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b7 ==> BV32_SLT($g1.0$2, $length) ==> p0$2;
  ...
  __partitioned_block_$3_0:
    ...
    assert {:tag "nowrite"} _b44 ==> !_WRITE_HAS_OCCURRED_$$c;
    ...
    assert {:tag "predicatedEquality"} _b34 ==> p2$1 && p2$2 ==> $g3.0$1 == $g3.0$2;
    assert {:tag "loopPredicateEquality"} _b33 ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 ==> p2$1 == p2$2;
    assert {:tag "loopPredicateEquality"} _b32 ==> p2$1 == p2$2;
    assert p2$1 ==> p0$1;
    assert p2$2 ==> p0$2;
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 1} p2$1 ==> _b16 ==> p2$1 ==> BV32_SLT($g1.0$1, $length);
    assert {:tag "conditionsImpliedByEnabledness"} {:thread 2} p2$2 ==> _b16 ==> p2$2 ==> BV32_SLT($g1.0$2, $length);
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b15 ==> BV32_SLT($g1.0$1, $length) && BV32_SLT($g3.0$1, $length) ==> p2$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b15 ==> BV32_SLT($g1.0$2, $length) && BV32_SLT($g3.0$2, $length) ==> p2$2;
