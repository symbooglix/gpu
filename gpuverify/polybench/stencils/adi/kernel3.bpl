...
implementation {:source_name "kernel3"} {:kernel} $kernel3($n: bv32, $tsteps: bv32, $h0: bv32)
{
  ...
  __partitioned_block_$1_0:
    ...
    assert {:tag "accessBreak"} _b24 ==> _WRITE_HAS_OCCURRED_$$X ==> local_id_x$1 == BV32_DIV(_WATCHED_OFFSET, $n);
    assert {:tag "accessBreak"} _b23 ==> _READ_HAS_OCCURRED_$$X ==> local_id_x$1 == BV32_DIV(_WATCHED_OFFSET, $n);
    assert {:tag "nowrite"} _b22 ==> !_WRITE_HAS_OCCURRED_$$X;
    assert {:tag "noread"} _b21 ==> !_READ_HAS_OCCURRED_$$X;
