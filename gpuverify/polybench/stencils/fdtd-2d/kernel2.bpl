...
implementation {:source_name "kernel2"} {:kernel} $kernel2($ny: bv32, $tmax: bv32, $nx: bv32, $h0: bv32)
{
  ...
  __partitioned_block_$3_0:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b19 ==> _READ_HAS_OCCURRED_$$kernel2.shared__fict_ ==> _WATCHED_OFFSET == 0bv32;
    assert {:tag "noread"} _b18 ==> !_READ_HAS_OCCURRED_$$kernel2.shared__fict_;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b17 ==> _WRITE_HAS_OCCURRED_$$ey ==> BV32_AND(BV32_SUB(1048576bv32, 1bv32), _WATCHED_OFFSET) == BV32_AND(BV32_SUB(1048576bv32, 1bv32), BV32_ADD(BV32_ADD(BV32_MUL(32bv32, group_id_x$1), local_id_x$1), BV32_MUL(0bv32, $ny)));
    assert {:tag "nowrite"} _b16 ==> !_WRITE_HAS_OCCURRED_$$ey;
    assert {:tag "predicatedEquality"} _b15 ==> p2$1 && p2$2 ==> v7$1 == v7$2;
    assert {:tag "predicatedEquality"} _b14 ==> p2$1 && p2$2 ==> v6$1 == v6$2;
    assert {:tag "predicatedEquality"} _b13 ==> p2$1 && p2$2 ==> v5$1 == v5$2;
    assert {:tag "predicatedEquality"} _b12 ==> p2$1 && p2$2 ==> v4$1 == v4$2;
    assert {:tag "predicatedEquality"} _b11 ==> p2$1 && p2$2 ==> v3$1 == v3$2;
    assert {:tag "predicatedEquality"} _b10 ==> p2$1 && p2$2 ==> $g1.0$1 == $g1.0$2;
    assert {:tag "loopPredicateEquality"} _b9 ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2 ==> p2$1 == p2$2;
    assert {:tag "loopPredicateEquality"} _b8 ==> p2$1 == p2$2;
    ...
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 1} _b7 ==> BV32_SLT($g1.0$1, $ny) ==> p2$1;
    assert {:do_not_predicate} {:tag "conditionsImplyingEnabledness"} {:thread 2} _b7 ==> BV32_SLT($g1.0$2, $ny) ==> p2$2;
