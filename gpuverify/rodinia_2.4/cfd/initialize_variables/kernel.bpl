...
implementation {:source_name "initialize_variables"} {:kernel} $initialize_variables($nelr: bv32)
{
  ...
  $1:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b7 ==> _WRITE_HAS_OCCURRED_$$variables ==> BV32_AND(BV32_SUB(BV32_MUL(1bv32, $nelr), 1bv32), _WATCHED_OFFSET) == BV32_AND(BV32_SUB(BV32_MUL(1bv32, $nelr), 1bv32), BV32_ADD(BV32_MUL(0bv32, $nelr), BV32_ADD(BV32_MUL(group_size_x, group_id_x$1), local_id_x$1)));
    assert {:tag "accessUpperBoundBlock"} _b6 ==> _WRITE_HAS_OCCURRED_$$variables ==> BV32_SLT(_WATCHED_OFFSET, BV32_ADD(BV32_MUL(group_size_x, BV32_ADD(group_id_x$1, 1bv32)), local_id_x$1));
    assert {:tag "accessLowerBoundBlock"} _b5 ==> _WRITE_HAS_OCCURRED_$$variables ==> BV32_SLE(BV32_ADD(BV32_MUL(group_size_x, group_id_x$1), local_id_x$1), _WATCHED_OFFSET);
