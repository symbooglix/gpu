//pass
//--local_size=[8,8,2] --num_groups=[44,11,1]
...
{
  __requires(*NbrListLen == __other_int(*NbrListLen));
  __requires(h == 0.5f);
  ...
  for (totalbins = 0;  totalbins < *NbrListLen;  totalbins += numbins) {
    ...
    for (bincnt = 0;
         __global_invariant(__write_implies(AtomBinCache,
                    ((__write_offset_bytes(AtomBinCache)/sizeof(float) - (startoff + (tid & 15)))%(8*BIN_SIZE) == 0) |
                    ((__write_offset_bytes(AtomBinCache)/sizeof(float) - (startoff + (tid & 15) + 16))%(8*BIN_SIZE) == 0))),
         bincnt < 4 && nbrid < *NbrListLen;  bincnt++, nbrid += 8) {
