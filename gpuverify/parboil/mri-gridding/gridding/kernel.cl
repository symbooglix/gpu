//pass
//--local_size=[8,4,2] --num_groups=[32,8192,1]
...
__kernel void gridding_GPU (__global ReconstructionSample* sample_g, 
  ...
  for (int z = zL;
       __invariant(__implies(__same_group, z == __other_int(z))),
       z <= zH; z++){
    for (int y = yL;
         __invariant(__implies(__same_group, y == __other_int(y))),
         y <= yH; y++){
      ...
      for (int x = 0;
           __invariant(__implies(__same_group, x == __other_int(x))),
           x < ((delta+TILE-1)>>LOG_TILE); x++){
