...
implementation {:source_name "writeLocalMemory"} {:kernel} $writeLocalMemory($size: bv32)
{
  ...
  $5:
    ...
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b11 ==> _READ_HAS_OCCURRED_$$writeLocalMemory.lbuf ==> _WATCHED_OFFSET == local_id_x$1;
    assert {:tag "accessedOffsetsSatisfyPredicates"} _b10 ==> _WRITE_HAS_OCCURRED_$$output ==> _WATCHED_OFFSET == BV32_ADD(BV32_MUL(group_size_x, group_id_x$1), local_id_x$1);
    assert {:tag "groupSharedArraysDisjointAcrossGroups"} _ATOMIC_HAS_OCCURRED_$$writeLocalMemory.lbuf ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2;
    assert {:tag "groupSharedArraysDisjointAcrossGroups"} _WRITE_HAS_OCCURRED_$$writeLocalMemory.lbuf ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2;
    assert {:tag "groupSharedArraysDisjointAcrossGroups"} _READ_HAS_OCCURRED_$$writeLocalMemory.lbuf ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2;
    assert {:tag "loopBound"} {:thread 1} _b9 ==> BV32_UGE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 2} _b9 ==> BV32_UGE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 1} _b8 ==> BV32_ULE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 2} _b8 ==> BV32_ULE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 1} _b7 ==> BV32_SGE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 2} _b7 ==> BV32_SGE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 1} _b6 ==> BV32_SLE($j.1, 0bv32);
    assert {:tag "loopBound"} {:thread 2} _b6 ==> BV32_SLE($j.1, 0bv32);
    assert {:tag "guardNonNeg"} {:thread 1} _b5 ==> BV32_SLE(0bv32, $j.1);
    assert {:tag "guardNonNeg"} {:thread 2} _b5 ==> BV32_SLE(0bv32, $j.1);
