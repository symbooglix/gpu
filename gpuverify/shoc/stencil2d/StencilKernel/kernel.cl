                ...
                __local VALTYPE* sh )
{
    ...
    if( lidCol == 0 )
    {
        for( int i = 0;
             __global_invariant(__write_implies(sh,
                                          (((__write_offset_bytes(sh)/sizeof(VALTYPE) - (lidCol + 1))/(lRowWidth + 2)) - lidRow) < (lszRow + 2)
                                        | (((__write_offset_bytes(sh)/sizeof(VALTYPE) -  lidCol     )/(lRowWidth + 2)) - lidRow) < (lszRow + 2)
                               )),
             __global_invariant(__write_implies(sh,
    ...
    else if( lidCol == (lszCol - 1) )
    {
        for( int i = 0;
             __global_invariant(__write_implies(sh,
                                          (((__write_offset_bytes(sh)/sizeof(VALTYPE) - (lidCol + 1))/(lRowWidth + 2)) - lidRow) < (lszRow + 2)
                                        | (((__write_offset_bytes(sh)/sizeof(VALTYPE) - (lidCol + 2))/(lRowWidth + 2)) - lidRow) < (lszRow + 2)
                                        | (((__write_offset_bytes(sh)/sizeof(VALTYPE) -  lidCol     )/(lRowWidth + 2)) - lidRow) < (lszRow + 2)
                               )),
             __global_invariant(__write_implies(sh,
                                          ((__write_offset_bytes(sh)/sizeof(VALTYPE) - (lidCol + 1))%(lRowWidth + 2)) == 0
