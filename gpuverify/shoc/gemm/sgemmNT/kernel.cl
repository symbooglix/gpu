                       ...
                       FPTYPE alpha, FPTYPE beta )
{
  __requires(ldc == 256);
    ...
    for( int i = 0;
         ...
         __global_invariant(__write_implies(C, (__write_offset_bytes(C)/sizeof(FPTYPE) - (ibx + id + iby * ldc))%ldc == 0)),
         __global_invariant(__read_implies(C, (__read_offset_bytes(C)/sizeof(FPTYPE) - (ibx + id + iby * ldc))/ldc < 16)),
         __global_invariant(__read_implies(C, (__read_offset_bytes(C)/sizeof(FPTYPE) - (ibx + id + iby * ldc))%ldc == 0)),
        i < 16; i++, C += ldc ){
