...
// #pragma unroll 22
    for (unsigned k=1;
         __global_invariant(__write_implies(C, __write_offset_bytes(C)/sizeof(real)/N_GP < 22)),
         __global_invariant(__write_implies(C, __write_offset_bytes(C)/sizeof(real)%N_GP == thread_num)),
         __global_invariant(__read_implies(C, __read_offset_bytes(C)/sizeof(real)/N_GP < 22)),
         __global_invariant(__read_implies(C, __read_offset_bytes(C)/sizeof(real)%N_GP == thread_num)),
         k<=22; k++)
    {
