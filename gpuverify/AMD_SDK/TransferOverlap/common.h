...
{                                                                         
   ...
   for(int n = 0;
       __invariant(__write_implies(out, ((__write_offset_bytes(out)/sizeof(uint4))%get_global_size(0)) == get_global_id(0))),
       n < nk; n++)                                                
   {
      for(i = 0, idx = get_global_id(0);
          __invariant((idx % get_global_size(0)) == get_global_id(0)),
          __invariant(__write_implies(out, ((__write_offset_bytes(out)/sizeof(uint4))%get_global_size(0)) == get_global_id(0))),
          i < ni; i++, idx += get_global_size(0))
      {                                                                   
          out[idx] = pval;                                                 
      }                                                                   
   }
