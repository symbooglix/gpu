...
{
  __requires(width == 512 /*get_global_size(0)*/);
  __requires(height == 512 /*get_global_size(1)*/);
  __requires(blockSize == 2 /*get_local_size(0)*/);
				       ...
				       const float coefp, const float coefn)
{
    ...
    for (int y = 0;
         __invariant(x >= 0),
         __invariant(__write_implies(output, ((__write_offset_bytes(output)/sizeof(uchar4))%width)==x)),
         __invariant(y >= 0),
         __invariant(y <= height),
         __invariant(__uniform_int(y)),
    ...
    for (int y = height - 1;
         __invariant(__write_implies(output, ((__write_offset_bytes(output)/sizeof(uchar4))%width)==x)),
         __invariant(__read_implies(output, ((__read_offset_bytes(output)/sizeof(uchar4))%width)==x)),
         __invariant(y >= -1),
         __invariant(y <= height - 1),
         __invariant(__uniform_int(y)),
         __invariant(__uniform_bool(__enabled())),
         y > -1; y--) 
    {
