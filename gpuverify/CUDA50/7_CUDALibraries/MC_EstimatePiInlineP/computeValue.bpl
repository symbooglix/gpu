...
implementation {:source_name "computeValue<float>"} {:kernel} $_Z12computeValueIfEvPjP17curandStateXORWOWj($numSims: bv32)
{
  ...
  $7:
    ...
    assert {:tag "pow2NotZero"} _b5 ==> $s.i.0 != 0bv32;
    assert {:tag "pow2"} _b4 ==> $s.i.0 == 0bv32 || BV32_AND($s.i.0, BV32_SUB($s.i.0, 1bv32)) == 0bv32;
    assert {:tag "groupSharedArraysDisjointAcrossGroups"} _ATOMIC_HAS_OCCURRED_$$sdata ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2;
    assert {:tag "groupSharedArraysDisjointAcrossGroups"} _WRITE_HAS_OCCURRED_$$sdata ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2;
    assert {:tag "groupSharedArraysDisjointAcrossGroups"} _READ_HAS_OCCURRED_$$sdata ==> group_id_x$1 == group_id_x$2 && group_id_y$1 == group_id_y$2 && group_id_z$1 == group_id_z$2;
    assert {:tag "loopBound"} {:thread 1} _b3 ==> BV32_UGE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 2} _b3 ==> BV32_UGE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 1} _b2 ==> BV32_ULE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 2} _b2 ==> BV32_ULE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 1} _b1 ==> BV32_SGE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 2} _b1 ==> BV32_SGE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 1} _b0 ==> BV32_SLE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
    assert {:tag "loopBound"} {:thread 2} _b0 ==> BV32_SLE($s.i.0, BV32_UDIV(group_size_x, 2bv32));
