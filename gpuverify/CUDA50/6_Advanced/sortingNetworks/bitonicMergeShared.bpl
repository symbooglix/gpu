...
implementation {:source_name "bitonicMergeShared"} {:kernel} $_Z18bitonicMergeSharedPjS_S_S_jjj($arrayLength: bv32, $size: bv32, $dir: bv32)
{
  ...
  $1:
    ...
    assert {:tag "loopBound"} {:thread 1} _b3 ==> BV32_UGE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 2} _b3 ==> BV32_UGE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 1} _b2 ==> BV32_ULE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 2} _b2 ==> BV32_ULE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 1} _b1 ==> BV32_SGE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 2} _b1 ==> BV32_SGE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 1} _b0 ==> BV32_SLE($stride.0, 512bv32);
    assert {:tag "loopBound"} {:thread 2} _b0 ==> BV32_SLE($stride.0, 512bv32);
