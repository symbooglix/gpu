...
procedure {:source_name "oddEvenMergeGlobal"} {:kernel} $_Z18oddEvenMergeGlobalPjS_S_S_jjjj($arrayLength: bv32, $size: bv32, $stride: bv32, $dir: bv32);
  requires {:sourceloc_num 0} {:thread 1} (if $arrayLength == 2048bv32 then 1bv1 else 0bv1) != 0bv1;
  requires {:sourceloc_num 1} {:thread 1} (if $stride == 1024bv32 then 1bv1 else 0bv1) != 0bv1;
...
implementation {:source_name "oddEvenMergeGlobal"} {:kernel} $_Z18oddEvenMergeGlobalPjS_S_S_jjjj($arrayLength: bv32, $size: bv32, $stride: bv32, $dir: bv32)
{
  var $valB.0$1: bv32;
  var $valB.0$2: bv32;
  var $keyB.0$1: bv32;
  ...
  $0:
    ...
    v1$1 := BV32_SUB(BV32_MUL(2bv32, v0$1), BV32_AND(v0$1, BV32_SUB($stride, 1bv32)));
    v1$2 := BV32_SUB(BV32_MUL(2bv32, v0$2), BV32_AND(v0$2, BV32_SUB($stride, 1bv32)));
    v2 := BV32_ULT($stride, BV32_UDIV($size, 2bv32));
    p0$1 := false;
    p0$2 := false;
    p1$1 := false;
    p1$2 := false;
  ...
  $falsebb:
    ...
    havoc v12$1, v12$2;
    v13$1 := BV1_ZEXT32((if BV32_UGT(v9$1, v11$1) then 1bv1 else 0bv1)) == $dir;
    v13$2 := BV1_ZEXT32((if BV32_UGT(v9$2, v11$2) then 1bv1 else 0bv1)) == $dir;
  ...
  $truebb:
    assume {:partition} v2 && v2;
    v3$1 := BV32_UGE(BV32_AND(v0$1, BV32_SUB(BV32_UDIV($size, 2bv32), 1bv32)), $stride);
    v3$2 := BV32_UGE(BV32_AND(v0$2, BV32_SUB(BV32_UDIV($size, 2bv32), 1bv32)), $stride);
    ...
    v8$1 := (if p0$1 then BV1_ZEXT32((if BV32_UGT(v4$1, v6$1) then 1bv1 else 0bv1)) == $dir else v8$1);
    v8$2 := (if p0$2 then BV1_ZEXT32((if BV32_UGT(v4$2, v6$2) then 1bv1 else 0bv1)) == $dir else v8$2);
